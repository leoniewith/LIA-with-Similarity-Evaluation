function [flag, positionindex, inter_index] = trytest2(x,GenotypeLength,VarietyNumber,XVessel,YVessel,RVessel,MaxRobotNo,PortPosition)
...
% 加入单个路径的自交叉
inter_index = 0;  % 初始化自交叉index为0
for i = 1:MaxRobotNo
    if length(coortemp{i}) ~= 0
        temp_index = intersection(coortemp{i});
        if temp_index == 1
            inter_index = 1;
        end
    end
